...
try {
  ...
  if (checkSets.length !== 4 || checks.length !== 21) {
    errors.push(
      `Expected 4 Check Sets and 21 Checks; found ${checkSets.length} and ${checks.length}.`
    );
  }
  ...
  if (errors.length) {
    ...
    console.log(
      `Package artifact audit passed: ${manifestMembers.size} Custom Metadata records ` +
        `(4 Check Sets, 21 Checks) exist in both package.xml and the physical artifact.`
    );
  }
} finally {
