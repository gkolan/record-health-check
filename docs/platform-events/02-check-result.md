...
> [!NOTE]
> Use this page to subscribe to `Record_Health_Check_Result__e` with Flow or Apex and route finalized Check outcomes without exposing values that the event intentionally omits.
...
setting. Review the [event metadata reference](../metadata/04-event-check-result.md) for publication
...
```apex
trigger RhcCheckResultSubscriber on Record_Health_Check_Result__e (
  after insert
) {
...
public with sharing class RhcCheckResultSubscriberHandler {
  public static void handle(
    ...
    Map<String, Record_Health_Check_Result__e> newByEventId =
      new Map<String, Record_Health_Check_Result__e>();
...
- [Check Result metadata](../metadata/04-event-check-result.md)
