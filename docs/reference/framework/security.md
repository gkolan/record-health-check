...
permission test data described in [Verify an Apex Check plugin](../apex/plugin-verification.md)
...
| --- | --- | --- |
| **Record Health Check User** (`rhc__Record_Health_Check_User`) | The **Record Health Check Run** (`rhc__Record_Health_Check_Run`) Custom Permission, needed to call the protected Apex API and run checks | Every user who should see the card |
| **Record Health Check Admin** (`rhc__Record_Health_Check_Admin`) | Everything in **Record Health Check User** (`rhc__Record_Health_Check_User`), plus the **Record Health Check View Diagnostics** (`rhc__Record_Health_Check_View_Diagnostics`) Custom Permission and the metadata validation access | Check authors and troubleshooters only |

Do not assign **Record Health Check Admin** (`rhc__Record_Health_Check_Admin`) to users who only need to run checks. Diagnostic detail
...
2. The running user holds the **Record Health Check View Diagnostics** (`rhc__Record_Health_Check_View_Diagnostics`) Custom Permission (included in
   **Record Health Check Admin** (`rhc__Record_Health_Check_Admin`)).
...
- [Verify an Apex Check plugin](../apex/plugin-verification.md)
