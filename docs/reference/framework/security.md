...
A custom Apex Rule plugin (`RecordHealthCheckRule`) is trusted code: the Framework cannot sandbox or
observe every read it performs. It can, however, guarantee that a plugin call has no side effect on
the transaction.
...
| --- | --- |
| DML of any kind | Thrown contract fault, `PLUGIN_SIDE_EFFECT_DETECTED` |
| Callouts | Thrown contract fault, `PLUGIN_SIDE_EFFECT_DETECTED` |
| Email | Thrown contract fault, `PLUGIN_SIDE_EFFECT_DETECTED` |
| Platform event publication | Thrown contract fault, `PLUGIN_SIDE_EFFECT_DETECTED` |
| Asynchronous work (Queueable, Future, Batch, Scheduled) | Thrown contract fault, `PLUGIN_SIDE_EFFECT_DETECTED` |
...
The side-effect fence is not a read-security boundary. A plugin must still use `with sharing` and
...
Access is split into two Permission Sets so run entitlement and troubleshooting access can be
...
diagnostics, gated by the same **Show Diagnostics** and Custom Permission combination described
above.
