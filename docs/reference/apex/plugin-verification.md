...
> [!NOTE]
> On this page, verify an Apex Check plugin's bulk behavior, access mode, returned outcomes, and
> prohibited writes before using it in a Check.

The bulk interface makes efficient plugins possible. It cannot inspect a subscriber's query loop
or determine the access mode used by custom Apex at runtime.
