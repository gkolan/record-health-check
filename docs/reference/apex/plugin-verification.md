...
> query loop or determine the access mode used by opaque Apex at runtime.
...
The harness checks scopes of 1, 10, 50, and 200 records. It checks approximate query
stability, complete result coverage, prohibited side effects, and an optional permission
fixture. The shipped `AccountHasRecentActivityCheck` passes it. The self-test also includes
...
- Behavioral evidence comes from a fixture where a least-privilege user cannot read a
...
- Opaque subscriber Apex has no runtime access-mode counter. Neither the interface nor
...
wired to the reusable harness.

## Reporting rules

- Do not silently disable a Rule because telemetry looks suspicious. Fail clearly or use
  an explicit administrator setting.
- Do not infer a complexity class from one measurement. Report the measured resource
  change and whether the Rule has passed the multi-size harness.
