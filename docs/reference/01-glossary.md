...
| --- | --- | --- |
| Formula | Fields on the current record or a reachable parent record | [Reference: Formula](evaluation/01-formula.md) |
| Query | One administrator-authored SOQL query over related records | [Reference: Query](evaluation/02-query.md) |
| Compare Two Queries | Two independent SOQL queries compared to each other | [Reference: Compare two queries](evaluation/03-compare-two-queries.md) |
| Apex | A class implementing `RecordHealthCheckPlugin` | [Reference: Apex](evaluation/04-apex-check-contract.md) |
...
administrator-authored display text. See [Reason Codes](contracts/01-reason-codes.md).
...
[Configuration identity](framework/06-configuration-identity.md).
...
[Configuration identity: Keep Demo starter configuration explicit](framework/06-configuration-identity.md#keep-demo-starter-configuration-explicit).
...
`{!record.Name fallback="this record"}`. See [Reference: Merge tokens](contracts/02-merge-tokens.md).
...
- [Reference: Architecture](framework/01-architecture.md)
