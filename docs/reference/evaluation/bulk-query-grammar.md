...
| `ORDERED_PICK_IN_MEMORY` | A record-correlated query with outer `ORDER BY` fields and `LIMIT N`, whether the selected field matches the ordered field, differs from it, or is a relationship path | Remove the per-record limit, fetch bounded candidate rows in one scope query, and choose up to N ordered rows per record in Apex. |
...
Negated correlation and non-equality correlation are unclassified. Ordered picks support multiple
order fields and per-record limits greater than one. Without an outer `ORDER BY`, a correlated
`LIMIT` greater than one is unsupported, except for a self query whose `Id` equality already
restricts it to one record. Child-subquery ordering and limits do not select the outer strategy.
