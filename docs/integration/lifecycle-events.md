...
> [Build a subscriber](../platform-events/README.md) ·
...
| One result for each selected Check | [`Record_Health_Check_Result__e`](../metadata/event-check-result.md) | Check **Publish User Result Event** |
...
sequenceDiagram
    ...
    participant Subscriber as Flow, Apex, or external subscriber

    Caller->>RHC: Run a Check or Check Set deliberately
    RHC->>RHC: Evaluate and construct the synchronous response
    RHC->>TX: Request lifecycle-event publication
    RHC-->>Caller: Return the synchronous response
    alt Transaction commits
        TX-)Bus: Publish accepted lifecycle events
        Bus-)Subscriber: Deliver events asynchronously
        alt Subscriber succeeds
            Subscriber->>Subscriber: Store or act on the result
        else Subscriber fails
            Subscriber->>Subscriber: Record failure and recover independently
        end
    else Transaction rolls back
        TX--xBus: Suppress Publish After Commit events
    end
    Note over Caller,Subscriber: Subscriber success never changes the completed health-check response
```

Text fallback:

```text
Caller -> Record Health Check -> synchronous response
                         |
                         +-> transaction commits -> event bus -> subscriber
                         |
                         +-> transaction rolls back -> no lifecycle event

Subscriber failure is monitored and recovered separately from the original response.
```
...
publication mode other than `NONE` and Check Set / Check metadata permits the event. A subscriber
must prevent feedback loops and should not start another publishing run from the event handler.

Keeping page-load publication off protects platform-event allocations. Subscriber designs must
prevent one event from starting another publishing run indefinitely. Each published event still
...
- [External Pub/Sub API subscriber](../platform-events/external-pub-sub-api.md)
