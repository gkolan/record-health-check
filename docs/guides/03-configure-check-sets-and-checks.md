...
| Guide users toward a correction | [Configure action links](04-configure-action-links.md) | Add Fix Message, Action Label, and a safe Action URL to failed Checks |
...
| [3. Check Set fields](#3-check-set-fields) | Link to [Check Set field reference](../metadata/01-fields-check-set.md) |
| [4. Check fields](#4-check-fields) | Link to [Check field reference](../metadata/02-fields-check.md) and action links |
...
For setup, see [Create your first Check](../installation/03-create-your-first-check.md). For action-link patterns, see
[Configure action links](04-configure-action-links.md). For troubleshooting detail, see
...
Every field on **Record Health Check** (`Record_Health_Check__mdt`) is documented in **[Check fields](../metadata/02-fields-check.md)**. Optional **Category** is metadata-only for now; the current card does not group rows by it. Optional **Fix Message**, **Action Label**, and **Action URL** fields render guidance and navigation on failed checks. Examples: [Configure action links](04-configure-action-links.md).
...
[Formula reference](../reference/evaluation/01-formula.md) for the complete contract.
...
Primary contract: [Display value format](../reference/contracts/03-display-value-format.md). Field
catalog: [Display: Value Format](../metadata/02-fields-check.md#display-value-format-displayvalueformat__c).
...
[Customer handoff](../examples/query/01-customer-contact.md); use the [Query reference](../reference/evaluation/02-query.md)
...
[Compare two queries reference](../reference/evaluation/03-compare-two-queries.md) for the complete
contract.
...
[Apex reference](../reference/evaluation/04-apex-check-contract.md), [Recent Account activity](../examples/apex/01-recent-activity.md),
...
| ----------- | -------- | ---- |
| Apex Class | `ApexClass__c` | Class implementing `RecordHealthCheckPlugin` |
| Apex Parameters (JSON) | `ApexParametersJson__c` | Optional configuration values passed as `context.parameters` |

For AI-assisted drafting, see [LLM Configuration Guide: Apex](06-draft-configuration-with-ai.md#54-apex-evaluationtype__c--apex) and [recent-activity Apex pattern](06-draft-configuration-with-ai.md#106-recent-taskevent-activity-apex-multi-object).
...
fallback, and limit contract, see the [Merge-token reference](../reference/contracts/02-merge-tokens.md).
...
| Custom automation runs slowly or hits limits | The request includes too many records or the Check Set expands into too much work for the transaction | Keep one request within 200 records and one Check Set within the first 25 active Checks. Use `RecordHealthCheck.evaluate(...)` with a focused Check Set; see [Apex API](../api/01-apex-api.md) or [Flow actions](../integration/02-flow-actions.md). |
...
For Reason Codes, see [Reason Codes](../reference/contracts/01-reason-codes.md).
...
- [ ] **Display: Value Format** is Auto unless a comparison Check needs an explicit Number, Currency, Percent, Ratio as Percent, Checkbox, Date, Date/Time, Text, or Raw format ([Display value format](../reference/contracts/03-display-value-format.md)).
...
- [ ] Apex Checks reference deployed `RecordHealthCheck` implementations.
...
| --- | --- | --- |
| Lightning record page | The card loads definitions, evaluates Checks, and shows Pass / Fail / Skipped / Unable to Check / System Error | [Lightning component](../integration/01-lightning-component.md) |
| Apex | `RecordHealthCheck.evaluate(request)` returns a typed response for one Check or Check Set | [Apex API](../api/01-apex-api.md) |
| Flow | Packaged actions return status counts and Result JSON | [Flow actions](../integration/02-flow-actions.md) |
| Lifecycle events | Optional after-commit Set Run / Check Result events for deliberate runs | [Lifecycle events](../integration/03-lifecycle-events.md) |

For layers, limits, and class ownership, see [Architecture](../reference/framework/01-architecture.md).
...
- [Create your first Check](../installation/03-create-your-first-check.md): first install and first Check
- [Examples library](../examples/README.md): practical patterns by Evaluation Type
- [Architecture](../reference/framework/01-architecture.md): published Framework architecture and source ownership
- [Check Set fields](../metadata/01-fields-check-set.md) · [Check fields](../metadata/02-fields-check.md)
- [Apex API](../api/01-apex-api.md) · [Flow actions](../integration/02-flow-actions.md)