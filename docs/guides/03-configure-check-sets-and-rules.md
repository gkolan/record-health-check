...
| Guide users toward a correction | [Configure action links](04-configure-action-links.md) | Add Fix Message, Action Label, and a safe Action URL to failed Rules |
...
| [4. Rule fields](#4-rule-fields) | Link to [Rule field reference](../metadata/02-fields-check-rule.md) and action links |
...
catalog: [Display: Value Format](../metadata/02-fields-check-rule.md#display-value-format-displayvalueformat__c).
...
[Apex reference](../reference/evaluation/04-apex-rule-contract.md), [Recent Account activity](../examples/apex/01-recent-activity.md),
...
- [ ] **Display: Value Format** is Auto unless a comparison Rule needs an explicit Number, Currency, Percent, Ratio as Percent, Checkbox, Date, Date/Time, Text, or Raw format ([Display value format](../reference/contracts/03-display-value-format.md)).
...
| --- | --- | --- |
| Lightning record page | The card loads definitions, evaluates Rules, and shows Pass / Fail / Skipped / Unable to Check / System Error | [Lightning component](../integration/01-lightning-component.md) |
| Apex | `RecordHealthCheck.evaluate(request)` returns a typed response for one Rule or Check Set | [Apex API](../api/01-apex-api.md) |
| Flow | Packaged actions return status counts and Result JSON | [Flow actions](../integration/02-flow-actions.md) |
| Lifecycle events | Optional after-commit Set Run / Rule Result events for deliberate runs | [Lifecycle events](../integration/03-lifecycle-events.md) |
...
- [Check Set fields](../metadata/01-fields-check-set.md) · [Rule fields](../metadata/02-fields-check-rule.md)
