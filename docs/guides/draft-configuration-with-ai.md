...
> [Create your first Check](../installation/create-your-first-check.md) or the
> [Check Set](../metadata/fields-check-set.md) / [Check](../metadata/fields-check.md) field
> references. For human configuration teaching, start with
> [Configure Check Sets and Checks](configure-check-sets-and-checks.md).

This file is the single source for AI assistants translating business requirements into correct Custom Metadata configuration. Paste the output tables into Setup; see [Create your first Check: Step 2](../installation/create-your-first-check.md#step-2-create-the-check). For every field explained, see [Configure Check Sets and Checks](configure-check-sets-and-checks.md). For exact field behavior, use the [Check Set fields](../metadata/fields-check-set.md) and [Check fields](../metadata/fields-check.md) references. Write explanations in direct, plain language and define Salesforce terms when a reader may not know them.

## Choose how to use this page

| Goal | Start with | Stop after |
| --- | --- | --- |
| Give an AI assistant the complete Framework rules | [System prompt](#2-system-prompt-copy-into-a-gemini-gem-or-custom-gpt) | Copy the fenced prompt, then apply the required output template |
| Decide the Evaluation Type yourself | [Decision tree](#3-decision-tree) | Confirm the fields required by the selected Evaluation Type |
| Review an AI-generated proposal | [Supported combinations](#8-supported-vs-unsupported-combinations) | Finish with the limits, clarifying questions, and checklist |

The numbered sections after the system prompt are reference material. Read the sections needed for
the proposed Check rather than treating this page as one linear tutorial.
...
└─ Needs code or an unsupported Salesforce-data shape?
   └─ EvaluationType__c = APEX
      ApexClass__c = class implementing rhc.RecordHealthCheckPlugin
...
| ----- | ---- | ---------------------- | --- |
| 1 Multi-object | Task **or** Event activity for each Account | `AccountHasRecentActivityCheck` | `apex/recent-activity.md` |
| 2 Child aggregation | Same child must fail combined conditions | `AccountOpenOpportunityHealthCheck` | `apex/open-opportunity-health.md` |
| 3 Composite | Weighted score, one collapsed indicator | *(reference: user deploys)* | `apex/strategic-readiness.md` |
...
| `RunButtonDisplay__c` | Run Button Display | No | `LABEL_AND_ICON` (default), `LABEL_ONLY`, `ICON_ONLY`, or `HIDE`; use `HIDE` only with `RUN_ON_LOAD` |
| `RunButtonLabel__c` | Run Button Label | No | Blank for `Run`, or custom text up to 80 characters |
| `RerunButtonLabel__c` | Rerun Button Label | No | Blank for `Rerun`, or custom text up to 80 characters |
| `RunButtonIcon__c` | Run Button Icon | No | Blank for the built-in play glyph, or a Lightning icon such as `utility:refresh` |
...
| `ShowDiagnostics__c` | Show Diagnostics | No | `false` in production. When `true`, user also needs **Record Health Check View Diagnostics** (`rhc__Record_Health_Check_View_Diagnostics`) (from **Record Health Check Admin** (`rhc__Record_Health_Check_Admin`)). See [Show Diagnostics guide](troubleshoot-with-show-diagnostics.md). |
...
**Component wiring:** In Lightning App Builder, select the intended **Check Set** for the record page. The stored LWC property is `checkSetName` and holds the Check Set's `QualifiedApiName`; Apex controller methods receive that qualified identity. Leave **Run Button Display** at **Inherit** unless this component instance needs a presentation-only override. Never combine an effective `HIDE` override with `RUN_ON_REQUEST`.
...
When `EvaluationType__c` = `APEX`, add a section after the Check table. See [Apex reference](../reference/evaluation/apex-check-contract.md) for full patterns.
...
| Outcome | Return exactly one `rhc.RecordHealthCheckOutcome` per scoped record ID; attach Found/Expected values when they explain the verdict |
...
Full walkthroughs: [Apex examples](../examples/README.md#apex-examples) · [Recent Account activity](../examples/apex/recent-activity.md) · [Apex reference](../reference/evaluation/apex-check-contract.md)

| API field | Required | Notes |
| --- | --- | --- |
| `ApexClass__c` | Yes | Class implementing `rhc.RecordHealthCheckPlugin`: deploy before activating Check |
| `ApexParametersJson__c` | No | JSON **object** (not array), e.g. `{"daysBack": 90}`, `{"minDigits": 10}`, `{"staleDays": 30}` |

**Apex interface summary:** Full patterns: [Apex reference](../reference/evaluation/apex-check-contract.md).
...
global with sharing class AccountExampleCheck
  ...
  global Map<Id, rhc.RecordHealthCheckOutcome> evaluate(
    ...
    Map<Id, rhc.RecordHealthCheckOutcome> outcomesById =
    ...
    for (Id recordId : scope.recordIds) {
      ...
      outcomesById.put(
        ...
        String.isNotBlank(accountRecord?.Industry)
          ? rhc.RecordHealthCheckOutcome.pass('INDUSTRY_PRESENT')
          : rhc.RecordHealthCheckOutcome.fail('INDUSTRY_MISSING')
      );
    }
    return outcomesById;
  }
...
- Pair the Check with `rhc.RecordHealthCheckContractTest` so bulk, access, mutation, forbidden-write,
...
Recommend only the shipped class names listed above. For composite scoring, name a **new** class and include a Class sketch for implementation (see [example 3](https://github.com/gkolan/record-health-check/blob/main/docs/examples/apex/strategic-readiness.md)).
...
| Weighted readiness score | Apex | | Custom class: [apex/strategic-readiness.md](https://github.com/gkolan/record-health-check/blob/main/docs/examples/apex/strategic-readiness.md) |
...
| Org-wide batch audit | No packaged scheduler | Apex batch calling `rhc.RecordHealthCheck.evaluate(request)` |
...
This class ships with Record Health Check. See [Recent Account activity](https://github.com/gkolan/record-health-check/blob/main/docs/examples/apex/recent-activity.md).
...
Doc: [apex/open-opportunity-health.md](https://github.com/gkolan/record-health-check/blob/main/docs/examples/apex/open-opportunity-health.md).
...
Include a **Class sketch** when outputting this pattern. Full reference code: [apex/strategic-readiness.md](https://github.com/gkolan/record-health-check/blob/main/docs/examples/apex/strategic-readiness.md).
...
- [Configure Check Sets and Checks](configure-check-sets-and-checks.md): every Setup field explained
- [Configuration guide: what it can check](configure-check-sets-and-checks.md#2-what-it-can-check): when to use which Evaluation Type
...
- [Create your first Check](../installation/create-your-first-check.md): install and first Check
...
2. Add `configure-check-sets-and-checks.md` and `https://github.com/gkolan/record-health-check/blob/main/docs/examples/README.md` as secondary knowledge.
...
6. Link humans to [Create your first Check](../installation/create-your-first-check.md) for entering metadata in Setup.

## Related

- [Configure Check Sets and Checks](configure-check-sets-and-checks.md)
- [Metadata reference](../metadata/README.md)
- [Apex reference](../reference/evaluation/apex-check-contract.md)
