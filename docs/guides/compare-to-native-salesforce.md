...
| Stable status and Reason Code for automation | No, only pass/block | No, only match/no-match | Depends on Flow design | Yes: status and Reason Code, versioned independently of display text. See [Reason Codes](../reference/contracts/reason-codes.md) |
