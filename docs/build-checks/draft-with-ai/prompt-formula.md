...
- FormulaResultType__c declares the type the formulas this Check calculates with return: AUTO (the
  default), BOOLEAN, NUMBER, DATE, DATETIME, or TEXT. A Pass Condition returns a checkbox value, so
  BOOLEAN is the usual choice. Leave AUTO when unsure.
- DisplayFoundFormula__c and DisplayExpectedFormula__c are Salesforce formulas that change only
  ...
  They never change pass or fail, and FormulaResultType__c does not apply to them.
