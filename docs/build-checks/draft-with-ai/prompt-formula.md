...
fail. Confirm the draft against [Formula examples](../../examples/formula/README.md) and
...
FORMULA Check (Evaluation Type = Verify with a formula / FORMULA).
...
Rules for this Evaluation Type (EvaluationType__c = FORMULA):
- PassConditionFormula__c is required. It is a Salesforce formula that returns true (pass) or false
  (fail).
- Use Salesforce formula syntax only, such as AND(), OR(), NOT(), ISBLANK(), ISPICKVAL(), and
  CASE().
  Correct: AND(NOT(ISBLANK(BillingCity)), NOT(ISBLANK(BillingState)), NOT(ISBLANK(BillingCountry)))
  Wrong: NOT(ISBLANK(BillingCity)) && NOT(ISBLANK(BillingState)). The operators &&, ||, ==, and !=
  are Apex and JavaScript, not Salesforce formula syntax.
- FormulaResultType__c is AUTO (the field default), BOOLEAN, NUMBER, DATE, DATETIME, or TEXT. It is
  used only when a QUERY Check evaluates ExpectedRecordFormula__c or FindInListFormula__c as a
  comparison operand. It does not control this FORMULA Check's Pass Condition: that formula is
  always Boolean. Propose AUTO as the portable cross-type value.
- DisplayFoundFormula__c and DisplayExpectedFormula__c are Salesforce formulas that change only
  what the card shows. Fixed display text is double quoted: "City, State, and Country populated".
  They never change pass or fail, and FormulaResultType__c does not apply to them; each display
  formula uses AUTO return-type probing independently.
- Always propose DisplayFoundFormula__c on a Formula Check, and pair it with either
  DisplayExpectedFormula__c or DisplayExpectedText__c. Without them the Pass Condition is a
  checkbox, so the card can only show the reader true or false, which tells them nothing about
  their record. The formula turns that into the value they actually need to see. Every Formula
  Check shipped with the package sets one, in this shape:
  DisplayFoundFormula__c: IF(NOT(ISBLANK(Email)), "Email recorded", "Neither recorded")
  DisplayFoundFormula__c: BLANKVALUE(MailingCity, "Business location unknown")
  DisplayExpectedFormula__c: "Active"
- A formula reads the current record and formula-reachable parents, such as Parent.BillingCity. If
  the requirement needs a related-record count or list, say that QUERY or COMPARE_TWO_QUERIES is
  the right Evaluation Type rather than inventing formula syntax that does not exist.
- Plan every formula surface before activation. Record Health Check tokenizes field references with
  deterministic token offsets, resolves ordinary and colon-qualified relationship paths, and
  follows formula-field dependencies up to 10 levels. Preview reports FIELD_NOT_ACCESSIBLE when a
  real path is unavailable to the running principal and FIELD_NOT_RESOLVED when the schema path is
  unknown; never treat either result as a business FAIL.
- Treat $User, $Profile, $Setup, $Permission, and $CustomMetadata as unsupported formula globals
  for the checked-record contract. Do not use them to create execution-user-dependent behavior.
  Use an explicit field, applicability rule, Query Check, or Apex Check instead.
- AUTO return-type probing is bounded and disclosure-safe. Do not repeat raw compiler exceptions or
  formula source in user-facing output. Validate syntax, token/type mismatch, unsupported
  constructs, missing paths, inaccessible paths, null results, and the actual execution principal
  through Preview and sandbox tests.
- Omit from metadata: SourceQuery__c, SourceQueryField__c, ComparisonQuery__c, ComparisonQueryField__c,
  QueryResultHandling__c, ComparisonOperator__c, ExpectedValueSource__c, ExpectedFixedValue__c,
  ExpectedRecordFormula__c, ExpectedCurrencyIsoCode__c, FindInListFormula__c, NoRowsResult__c,
  EmptyValueHandling__c, MaxQueryRows__c, ApexClass__c, and ApexParametersJson__c.
- Example message: {!record.Name fallback="This Account"} is missing Billing City.
- Example Action URL: /lightning/r/Account/{!record.Id}/edit
```
