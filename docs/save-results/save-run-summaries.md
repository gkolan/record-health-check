...
Start with this event when summary counts are enough.
...
Use the [Check Result event](./save-individual-results.md) instead when receiving automation must know which
...
See the [complete event field reference](../reference/platform-event-metadata/check-set-run.md).
...
Check Admin** Permission Sets include access to this event. Give the Flow or Apex code separate
access to the history object and fields.
...
- [Record Health Check Set Run event fields](../reference/platform-event-metadata/check-set-run.md)
...
- [Use Batch Apex](../developer-guides/async-apex/batch.md)
