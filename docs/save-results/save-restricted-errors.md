# Save or route restricted errors
...
[Check Result event](./save-individual-results.md) for `FAIL`, `UNABLE_TO_EVALUATE`, and `ERROR` outcomes that
...
Check Admin** Permission Sets do not provide access to this event. Assign the packaged **Record
Health Check Error Log Publisher** Permission Set narrowly to each running user whose Check Sets
keep this setting enabled. It grants
...
See the [complete Log event field reference](../reference/platform-event-metadata/error-log.md).
...
- [Record Health Check Log event fields](../reference/platform-event-metadata/error-log.md)
